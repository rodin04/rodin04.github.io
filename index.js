...
function PLOTT() {
    ...
    const loeschen_label = document.createElement('div');
    loeschen_label.classList.add('data_labels');
    loeschen_label.textContent = `löschen`;
    loeschen_label.style.color = "red";
    ...
    container.appendChild(loeschen_label);
    ...
    */
    ...
    if(einheiten_bool)
    {
        for (let i = 0; i < anzahl_laufe; i++) 
        {
            ...
            const row_loeschen_button = document.createElement('button');
            row_loeschen_button.classList.add('cell2');
            row_loeschen_button.textContent = " X";
            row_loeschen_button.style.color = "red";

            // Event Listener mit Index
            row_loeschen_button.addEventListener('click', () => {
                ROW_LOESCHEN(i);
            });
            ...
            container.appendChild(row_loeschen_button);
        }
    }
    else
    {
    ...
    for (let i = 0; i < final_avgs.length; i++) 
    {
        ...
        if(i == 4)
        {
            labelGesamt.textContent = `${final_avgs[i].toFixed(0)} ${einheiten[i]}`;
...
function ROW_LOESCHEN(i)
{
    
    alert(i); // zeigt den Index an
    ...
    kmhList.splice(i, 1);
    paceList.splice(i, 1);
    pro100List.splice(i, 1);

    
    helper_list.splice(i, 1);
    timeSek_list.splice(i, 1);
